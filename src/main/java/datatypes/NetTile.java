package datatypes;

public class NetTile {

    public short Unknown0;
    public byte Unknown1;
    public byte Unknown2;
    public short Unknown3;
    public byte Unknown4;
    public short Unknown5;
    public byte Unknown6;
    public byte Unknown7;
    public short Unknown8;
    public byte Unknown9;
    public byte Unknown10;
    public byte Unknown11;
    public byte Unknown12;
    public byte Unknown13;
    public byte LiquidLevel;
    public long Gravity;

    public NetTile()
    {
    }

	/**
	 * @return the unknown0
	 */
	public short getUnknown0()
	{
		return Unknown0;
	}

	/**
	 * @param unknown0 the unknown0 to set
	 */
	public void setUnknown0(short unknown0)
	{
		Unknown0 = unknown0;
	}

	/**
	 * @return the unknown1
	 */
	public byte getUnknown1()
	{
		return Unknown1;
	}

	/**
	 * @param unknown1 the unknown1 to set
	 */
	public void setUnknown1(byte unknown1)
	{
		Unknown1 = unknown1;
	}

	/**
	 * @return the unknown2
	 */
	public byte getUnknown2()
	{
		return Unknown2;
	}

	/**
	 * @param unknown2 the unknown2 to set
	 */
	public void setUnknown2(byte unknown2)
	{
		Unknown2 = unknown2;
	}

	/**
	 * @return the unknown3
	 */
	public short getUnknown3()
	{
		return Unknown3;
	}

	/**
	 * @param unknown3 the unknown3 to set
	 */
	public void setUnknown3(short unknown3)
	{
		Unknown3 = unknown3;
	}

	/**
	 * @return the unknown4
	 */
	public byte getUnknown4()
	{
		return Unknown4;
	}

	/**
	 * @param unknown4 the unknown4 to set
	 */
	public void setUnknown4(byte unknown4)
	{
		Unknown4 = unknown4;
	}

	/**
	 * @return the unknown5
	 */
	public short getUnknown5()
	{
		return Unknown5;
	}

	/**
	 * @param unknown5 the unknown5 to set
	 */
	public void setUnknown5(short unknown5)
	{
		Unknown5 = unknown5;
	}

	/**
	 * @return the unknown6
	 */
	public byte getUnknown6()
	{
		return Unknown6;
	}

	/**
	 * @param unknown6 the unknown6 to set
	 */
	public void setUnknown6(byte unknown6)
	{
		Unknown6 = unknown6;
	}

	/**
	 * @return the unknown7
	 */
	public byte getUnknown7()
	{
		return Unknown7;
	}

	/**
	 * @param unknown7 the unknown7 to set
	 */
	public void setUnknown7(byte unknown7)
	{
		Unknown7 = unknown7;
	}

	/**
	 * @return the unknown8
	 */
	public short getUnknown8()
	{
		return Unknown8;
	}

	/**
	 * @param unknown8 the unknown8 to set
	 */
	public void setUnknown8(short unknown8)
	{
		Unknown8 = unknown8;
	}

	/**
	 * @return the unknown9
	 */
	public byte getUnknown9()
	{
		return Unknown9;
	}

	/**
	 * @param unknown9 the unknown9 to set
	 */
	public void setUnknown9(byte unknown9)
	{
		Unknown9 = unknown9;
	}

	/**
	 * @return the unknown10
	 */
	public byte getUnknown10()
	{
		return Unknown10;
	}

	/**
	 * @param unknown10 the unknown10 to set
	 */
	public void setUnknown10(byte unknown10)
	{
		Unknown10 = unknown10;
	}

	/**
	 * @return the unknown11
	 */
	public byte getUnknown11()
	{
		return Unknown11;
	}

	/**
	 * @param unknown11 the unknown11 to set
	 */
	public void setUnknown11(byte unknown11)
	{
		Unknown11 = unknown11;
	}

	/**
	 * @return the unknown12
	 */
	public byte getUnknown12()
	{
		return Unknown12;
	}

	/**
	 * @param unknown12 the unknown12 to set
	 */
	public void setUnknown12(byte unknown12)
	{
		Unknown12 = unknown12;
	}

	/**
	 * @return the unknown13
	 */
	public byte getUnknown13()
	{
		return Unknown13;
	}

	/**
	 * @param unknown13 the unknown13 to set
	 */
	public void setUnknown13(byte unknown13)
	{
		Unknown13 = unknown13;
	}

	/**
	 * @return the liquidLevel
	 */
	public byte getLiquidLevel()
	{
		return LiquidLevel;
	}

	/**
	 * @param liquidLevel the liquidLevel to set
	 */
	public void setLiquidLevel(byte liquidLevel)
	{
		LiquidLevel = liquidLevel;
	}

	/**
	 * @return the gravity
	 */
	public long getGravity()
	{
		return Gravity;
	}

	/**
	 * @param gravity the gravity to set
	 */
	public void setGravity(long gravity)
	{
		Gravity = gravity;
	}


}
